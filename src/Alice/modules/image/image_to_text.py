...
from Alice.core.exceptions import ImageToTextError
...
class ImageToText:
    ...
    async def analyze_image(self, image_data: Union[str, bytes], prompt: str = "") -> str:
        ...
        try:
            ...
            # Create the messages for the vision API using Llama format
            messages = [
                {
                    "role": "user",
                    "content": [
            ...
            self.logger.info(
                f"Sending request to Groq with model: {settings.ITT_MODEL_NAME}")
